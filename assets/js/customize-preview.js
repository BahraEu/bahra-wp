/* global bahraBgColors, bahraPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( bahraBgColors ).forEach( function( context ) {
					bahraGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function bahraGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'bahra-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#bahra-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( bahraPreviewEls[ context ] ) {
				_.each( bahraPreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		bahraGenerateColorA11yPreviewStyles( 'content' );
		bahraGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
