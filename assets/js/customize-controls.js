/* global bahraBgColors, bahraColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( bahraBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( bahraBgColors[ context ].color ) {
						backgroundColorValue = bahraBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( bahraBgColors[ context ].setting ).get();
					}
					bahraSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( bahraBgColors ).forEach( function( context ) {
			wp.customize( bahraBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					bahraSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		bahraSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				bahraSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function bahraSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = bahraColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function bahraSetRetineLogoVisibility( visible ) {
